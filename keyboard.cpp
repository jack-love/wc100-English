    ...
    ui(new Ui::KeyBoard)
{
  ...
  //  ui->lineEdit_keyboard->setValidator(validator);
   ...
   setFixedSize(480,800); // 禁止改变窗口大小。
    ...
    qDebug()<<"KeyBoard";
 ui->lineEdit_keyboard->setEnabled(false);
 lineEdit_keyboard=ui->lineEdit_keyboard;
...
void KeyBoard::focusChanged(QWidget *oldWidget, QWidget *nowWidget)
{
    ...
    //qDebug()<<oldWidget->hasFocus;
    if (nowWidget != 0 && !this->isAncestorOf(nowWidget)) {
        if (nowWidget->inherits("QLineEdit")) {
            bak_currentLineEdit = nowWidget;

           //  currentLineEdit = (QLineEdit *)nowWidget;
             lineEdit_keyboard->setText(((QLineEdit *)nowWidget)->text());
              currentLineEdit =lineEdit_keyboard;
            qDebug()<<"focusChanged---->1";
           this->setVisible(true);
            //OpenKeyboard();
        } else {
           currentLineEdit = 0;
            this->setVisible(false);
           qDebug()<<"focusChanged---->2";
//            //需要将输入法切换到最初的原始状态--小写
            //currentType="min";
...
void KeyBoard::setFilter(Filter_s  set) {

if(set == nu_s){number=true;letter=false;}
else if(set == le_s){letter=true; number=false;}
if(set==nule_s){number=true;letter=true;}
}
void KeyBoard::KeyboardInputCharacter(char c)
{
   if(number){
    if(c >='0' && c<='9'){printf("is number\n");
    return ;
    }
  }
   if(letter){
    if ((c >='a') &&( c<='z') || ( c >='A') && (c<='Z')){ printf("is Letter\n");
    return;
    }
 }
...
void KeyBoard::on_pushButton_yes_clicked()
{
 ...
 //   ui->lineEdit_keyboard->clear();
    accept();
    ((QLineEdit*)bak_currentLineEdit)->setText(ui->lineEdit_keyboard->text());
    //currentLineEdit->setText( ui->lineEdit_keyboard->text());
    this->setVisible(false);
    this->clearFocus();
}
