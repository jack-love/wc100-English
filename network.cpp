...
qDebug()<<"[getFileInfo2]"<<url<<path<<currentFileName;
    request.setUrl(url);
...
void Network::startDownload(QString task, QString path)
{
    ...
    QString path_tmp="/opt/wifi";

    if (isDownloading) {/* 如果任务正在下载，则返回 */
        return ;
    }
    ...
    QString fileName = path_tmp + "/" + QFileInfo(address).fileName();
qDebug()<<"[startDownload]------>"<<fileName<<path;
...
void Network::gotFileInfo()
{
    ...
    qDebug() << "[gotFileInfo]File name: " << currentGettingTask <<currentFileName << " size: " << fileSize;
...
{
    qDebug() << "downloadError ! " << code;
    emit downError_signal("failed");
    stopTask(currentTask);
}

void Network::stopDownload(){
    stopTask(currentTask);
}
